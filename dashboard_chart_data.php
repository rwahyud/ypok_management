...
foreach ($months as $m) {
    ...
    $stmt = $pdo->prepare("SELECT
        ...
        FROM transaksi WHERE TO_CHAR(created_at,'YYYY-MM') = ? AND status='lunas'");
...
// ── Pendaftaran ───────────────────────────────────────────────
$pendaftaran = ['labels' => [], 'msh' => [], 'kohai' => []];
foreach ($months as $m) {
    $pendaftaran['labels'][] = $m['label'];
    ...
    $stmt->execute([$m['param'], $m['param']]);
    $row = $stmt->fetch();
    $pendaftaran['msh'][]   = (int)$row['msh'];
    $pendaftaran['kohai'][] = (int)$row['kohai'];
}

// ── Prestasi pie (total all-time within year) ─────────────────
$stmt = $pdo->prepare("SELECT COUNT(*) FROM prestasi_msh   WHERE EXTRACT(YEAR FROM created_at) = ?");
$stmt->execute([$year]);
$pieMSH = (int)$stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM prestasi_kohai WHERE EXTRACT(YEAR FROM created_at) = ?");
$stmt->execute([$year]);
