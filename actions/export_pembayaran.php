...
if ($periode === 'month') {
    $where = "EXTRACT(MONTH FROM tanggal_bayar) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM tanggal_bayar) = EXTRACT(YEAR FROM CURRENT_DATE)";
    $periode_text = "Bulan " . date('F Y');
} elseif ($periode === 'last_month') {
    $where = "EXTRACT(MONTH FROM tanggal_bayar) = EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL '1 month') AND EXTRACT(YEAR FROM tanggal_bayar) = EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL '1 month')";
...
$sql_totals = "SELECT jenis_pembayaran, COALESCE(SUM(CASE WHEN status = 'lunas' THEN jumlah ELSE 0 END), 0) as total_lunas
               FROM pembayaran WHERE $where GROUP BY jenis_pembayaran";
...
$sql_total_all = "SELECT jenis_pembayaran, COALESCE(SUM(jumlah), 0) as total FROM pembayaran WHERE $where GROUP BY jenis_pembayaran";
...
if ($format === 'csv') {
    ...
    foreach($pembayaran_list as $p) {
        ...
        echo '<td>' . htmlspecialchars($p['jenis_pembayaran']) . '</td>';
        ...
        echo '<td>' . htmlspecialchars(ucfirst(str_replace('_', ' ', $p['status']))) . '</td>';
        echo '</tr>';
    }
                ...
                <tr>
                    ...
                    <td>
                        <span class="badge badge-<?php 
                            echo $p['jenis_pembayaran'] == 'Ujian' ? 'primary' : 
                                ($p['jenis_pembayaran'] == 'Kyu' ? 'info' : 
                                ($p['jenis_pembayaran'] == 'Rakernas' ? 'warning' : 'primary')); 
                        ?>">
                            <?php echo htmlspecialchars($p['jenis_pembayaran']); ?>
                        </span>
                    </td>
