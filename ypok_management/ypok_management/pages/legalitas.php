...
function updateLegalitasStatus($pdo) {
    ...
    $stmt = $pdo->prepare("UPDATE legalitas SET status = 'Kadaluarsa' WHERE tanggal_kadaluarsa < ? AND tanggal_kadaluarsa != '9999-12-31' AND is_permanent = 0 AND status != 'Dalam Proses'");
    $stmt->execute([$today]);
    
    // Update ke Akan Kadaluarsa (hanya untuk dokumen yang bukan permanent, bukan dalam proses, dan tanggal bukan 9999-12-31)
    $stmt = $pdo->prepare("UPDATE legalitas SET status = 'Akan Kadaluarsa' WHERE tanggal_kadaluarsa BETWEEN ? AND ? AND tanggal_kadaluarsa != '9999-12-31' AND is_permanent = 0 AND status != 'Dalam Proses'");
    $stmt->execute([$today, $warning_date]);
    
    // Update ke Aktif (jika masih jauh dari kadaluarsa, bukan permanent dan bukan dalam proses)
    $stmt = $pdo->prepare("UPDATE legalitas SET status = 'Aktif' WHERE tanggal_kadaluarsa > ? AND tanggal_kadaluarsa != '9999-12-31' AND is_permanent = 0 AND status != 'Dalam Proses'");
    $stmt->execute([$warning_date]);
    
    // Update dokumen permanent selalu aktif (kecuali dalam proses)
    $stmt = $pdo->prepare("UPDATE legalitas SET status = 'Aktif' WHERE is_permanent = 1 AND status != 'Dalam Proses'");
    $stmt->execute();
}
