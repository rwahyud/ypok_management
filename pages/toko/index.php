...
if($date_from && $date_to) {
    $stats_query .= " AND DATE(tanggal) BETWEEN '$date_from' AND '$date_to'";
}
...
if($search) {
    $transaksi_query .= " AND (p.nama_produk LIKE '%$search%' OR t.pembeli LIKE '%$search%' OR t.id_transaksi LIKE '%$search%')";
}

if($date_from && $date_to) {
    $transaksi_query .= " AND DATE(t.tanggal) BETWEEN '$date_from' AND '$date_to'";
}
...
<body>
    ...
    <script>
        ...
        function exportPDF() {
            window.open('actions/export_transaksi_pdf.php', '_blank');
        }
