...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if($_POST['action'] == 'add_kohai' || $_POST['action'] == 'edit_kohai') {
        ...
        if($_POST['action'] == 'edit_kohai') {
            ...
            $sql = "UPDATE kohai SET kode_kohai = ?, nama = ?, tempat_lahir = ?, tanggal_lahir = ?, 
                    jenis_kelamin = ?, tingkat_kyu = ?, sabuk = ?, dojo_cabang = ?, no_telp = ?, 
                    email = ?, nama_wali = ?, no_telp_wali = ?, status = ?, alamat = ?";
            $params = [$kode_kohai, $nama, $tempat_lahir, $tanggal_lahir, $jenis_kelamin, 
                      $tingkat_kyu, $sabuk, $dojo_cabang, $no_telp, $email, $nama_wali, 
                      $no_telp_wali, $status, $alamat];
            ...
            $stmt = $pdo->prepare("INSERT INTO kohai (kode_kohai, nama, tempat_lahir, tanggal_lahir, jenis_kelamin, tingkat_kyu, sabuk, dojo_cabang, no_telp, email, nama_wali, no_telp_wali, status, alamat, foto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            
            if($stmt->execute([$kode_kohai, $nama, $tempat_lahir, $tanggal_lahir, $jenis_kelamin, $tingkat_kyu, $sabuk, $dojo_cabang, $no_telp, $email, $nama_wali, $no_telp_wali, $status, $alamat, $foto])) {
...
if($search) {
    $stmt = $pdo->prepare("SELECT * FROM kohai $where ORDER BY created_at DESC, id ASC LIMIT $limit OFFSET $offset");
    $stmt->execute($params);
    ...
    $stmt = $pdo->query("SELECT * FROM kohai ORDER BY created_at DESC, id ASC LIMIT $limit OFFSET $offset");
    $kohai_list = $stmt->fetchAll();
            ...
            <div class="table-container">
                    ...
                    <tbody>
                        <?php if(count($kohai_list) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td><strong><?php echo htmlspecialchars($kohai['kode_kohai']); ?></strong></td>
                                <td><?php echo htmlspecialchars($kohai['nama']); ?></td>
                                <td><?php echo htmlspecialchars($kohai['tempat_lahir']) . ', ' . date('d F Y', strtotime($kohai['tanggal_lahir'])); ?></td>
                                <td><?php echo $kohai['jenis_kelamin'] == 'L' ? 'Laki-laki' : 'Perempuan'; ?></td>
                                <td style="text-align: center;">
                                    <span class="badge-kyu"><?php echo htmlspecialchars($kohai['tingkat_kyu']); ?></span>
                                </td>
                                <td style="text-align: center;">
                                    <span class="badge-sabuk <?php echo strtolower($kohai['sabuk']); ?>"><?php echo ucfirst($kohai['sabuk']); ?></span>
                                </td>
                                <td><?php echo htmlspecialchars($kohai['dojo_cabang']); ?></td>
                                <td><?php echo htmlspecialchars($kohai['no_telp']); ?></td>
                                <td><?php echo htmlspecialchars($kohai['email']); ?></td>
                                <td title="<?php echo htmlspecialchars($kohai['alamat']); ?>"><?php echo htmlspecialchars(substr($kohai['alamat'], 0, 25)) . ''; ?></td>    
                                <td style="text-align: center;">
                                    <span class="badge-<?php echo strtolower($kohai['status']); ?>"><?php echo ucfirst($kohai['status']); ?></span>
                                </td>
                                <td>
                ...
                <div class="pagination">
                    <div class="pagination-info">
                        <strong style="color: #1e40af;">📋 Menampilkan data <?php echo $offset + 1; ?>-<?php echo min($offset + $limit, $total); ?></strong> dari <strong style="color: #dc2626; font-size: 16px;">total <?php echo $total; ?> data Kohai</strong>
                    </div>
