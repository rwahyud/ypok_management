...
// Count by type
$count_pemasukan = $pdo->prepare("SELECT COUNT(*) FROM transaksi WHERE jenis='pemasukan'" . ($periode !== 'all' ? " AND TO_CHAR(tanggal, 'YYYY-MM') = ?" : ""));
if($periode !== 'all') {
...
$count_pengeluaran = $pdo->prepare("SELECT COUNT(*) FROM transaksi WHERE jenis='pengeluaran'" . ($periode !== 'all' ? " AND TO_CHAR(tanggal, 'YYYY-MM') = ?" : ""));
if($periode !== 'all') {
...
<body>
    ...
    <script>
        ...
        function handleExportSubmit(event) {
            ...
            let url = 'export_laporan_keuangan.php?';
            url += 'format=' + format;
            url += '&periode=' + periode;
